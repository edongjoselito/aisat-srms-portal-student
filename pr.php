...
<?php 
 $setting_set = mysqli_query($con, "SELECT * FROM srms_settings");
 $set = mysqli_fetch_assoc($setting_set);
?>
<br /><br />
<h1 class="textHeader">PROMOTIONAL REPORT</h1>
<h2 class="yl"><?php echo $sem; ?>, School Year <?php echo $sy; ?></h2>
<br /><br />

<div class="block"></div>
<div class="mainTable">
<button onclick="exportTableToExcel('tblData', 'pr_final')">Export To Excel</button>
<table cellspacing="0" cellpadding="0" id="tblData">
   <?php
     $sql  = "SELECT * FROM registration INNER JOIN grades ON registration.StudentNumber= grades.StudentNumber WHERE registration.SY='$sy' ";
     $sql .= "AND registration.Sem='$sem' ";
     $sql .= "GROUP BY registration.StudentNumber ";
  ...
  <tr>
    <td><?php echo $no; ?></td>
    <td><?php echo htmlentities($prof['LastName']); ?></td>
    ...
    <td  class="in">
      <table>
        <tr>
          <td><p><?php echo htmlentities($sub['Final']); ?></p></td>
        </tr>
      </table>
    </td>
    ...
    <td  class="in">
     ...
     $sql  = "SELECT sum(LecUnit) as lec FROM grades WHERE Semester='$sem' AND SY='$sy' AND StudentNumber='$id'";
     $lec_set = mysqli_query($con, $sql);
     $lec = mysqli_fetch_assoc($lec_set);
     // get the total sum of the lab unit column
     $sql  = "SELECT sum(LabUnit) as lab FROM grades WHERE Semester='$sem' AND SY='$sy' AND StudentNumber='$id'";
